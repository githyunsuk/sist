...
public class BoardUtil {
	 ...
	 * url - 이동할 URL, field-검색 필드(제목, 내용, 작성자), keyword-검색키워드를 pDTO로 입력받아서
	 * 쪽번호 를 생성하는 method 
	 * @param pDTO
	 * @return
	 */
	public static String pagination(PaginationDTO pDTO) {
		...
		if (pDTO.getCurrentPage() > pDTO.getPageNumber()) {// 시작게이지보다 1적은 페이지로 이동
			prevMark.delete(0, prevMark.length());
			movePage = startPage - 1;
			prevMark.append("[ <a href='").append(pDTO.getUrl()).append("?currentPage=").append(movePage)
					.append(searchQueryString.toString()).append("'class='pagePrevMark'>&lt;&lt;</a> ]");
		}
		...
		while (movePage <= endPage) {//
			if (movePage == pDTO.getCurrentPage()) {// 현재페이지는 링크 설정 x
				...
				pageLink.append("[ <a href='").append(pDTO.getUrl()).append("?currentPage=").append(movePage)
						.append(searchQueryString.toString()).append("'class='pageNotCurrent'>").append(movePage)
						.append("</a> ]");
			} // end else
			movePage++;
		} // end while
		...
		if (pDTO.getTotalPage() > endPage) {
			...
			nextMark.append("[ <a href='").append(pDTO.getUrl()).append("?currentPage=").append(movePage)
					.append(searchQueryString.toString()).append("' class='pageNextMark'>&gt;&gt;</a> ]");
		} // end if
